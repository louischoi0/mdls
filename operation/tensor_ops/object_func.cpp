...
void tfunc::init_tensor_with_random(tensor& c,int s , int e )
{
	...
	std::memcpy(c.get_p(), p, sizeof(elemt) *  B((&c)) * R((&c)) * D((&c)) * C((&c)));
}
...
void tfunc::compact_col(tensor* t, int size)
{
	...
	std::memcpy(t->get_p(), enp, sizeof(elemt) * B(t) * D(t) * dvc * R(t));
...
void tfunc::compact_channel(tensor* t, int size)
{
	...
	{
		std::memcpy(nnp , ppv , sizeof(elemt) * R(t) * C(t));
	...
	std::memcpy(ppv, enp, sizeof(elemt) * B(t) * C(t) * dvd * R(t));
...
{
	t.transpose(mdef::n1, mdef::n0, mdef::n2, mdef::n3);
...
void tfunc::tdot_n(tensor* t , tensor* tt , tensor& o)
{
	...
	loop::loop_tensor_with_same_chn_count(t, tt, o, 1,1,
...
void tfunc::af_inv_acm_chn(tensor* dout, tensor* in_tsp, tensor* weight_tsp , tensor& dout_in, tensor &dweight_in)
{
	...
	kintp.transpose(mdef::n0, mdef::n1, mdef::n3, mdef::n2);
	kwt.transpose(mdef::n0, mdef::n1, mdef::n3, mdef::n2);
...
void tfunc::tadd(tensor& in, tensor* e )
{

	loop::loop_tensor_with_same_chn_count( &in , e , in , 1,1,
		[](elemt* p, elemt* pp, elemt* nuse, const shape& s0, const shape& s1, const shape& s2, int n , int nn)->void
	{
		for (int i = 0; i < s0.Row * s0.Col; i++)
		{
			*p++ += *pp++;
