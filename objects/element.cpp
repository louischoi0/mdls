...
void element::reshape(const shape& s)
{
	...
	else
	{
		...
		std::memcpy(t, _pv, sizeof(elemt) * _eshape.Alloc_size);

		free(_pv);
		_pv = (elemt*)malloc(sizeof(elemt) * s.Alloc_size);

		std::memcpy(_pv, t, sizeof(elemt) * _eshape.Alloc_size);
...
void element::transpose(Dm n0, Dm n1, Dm n2, Dm n3)
{
	...
	(_pv, res, sizeof(elemt) * _eshape.Alloc_size);
	init(new_shape);
...
element::element(const element& t)
{
	...
	std::memcpy(_pv, t._pv, sizeof(elemt) * _eshape.Alloc_size);
}
...
void element::rotate(int rt)
{
	...
	std::memcpy(_pv, ppv, sizeof(elemt)* _eshape.Alloc_size);
	init(csh);
...
void element::apply_to_elem_by_batch(std::function<void(elemt* p, elemt* pp)> f, elemt * opts)
{
	elemt* p = _pv;
	elemt* op = opts;

	for (int j = 0; j < _eshape.Batch; j++)
	{
		p = _pv + (j * B_OFF(this));

		for (int i = 0; i < _eshape.Col * _eshape.Row * _eshape.Channel; i++)
		{
			f((p), opts);
			p++;
