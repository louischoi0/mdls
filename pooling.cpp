...
{
	shape ws(1, D(_tensor_map[Input]), _inf.stride_col, _inf.stride_col);

	int dvr = macro::get_dv_size(R(_tensor_map[Input]), ws.Row , _inf.stride_row);
	int dvc = macro::get_dv_size(C(_tensor_map[Input]), ws.Col , _inf.stride_col);
...
void pooling::initialize(int i, int ii)
{
}
