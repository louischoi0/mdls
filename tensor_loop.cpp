		...
		for (int i = 0; i < B(pp); i++)
		{
			...
			for (int z = 0; z < D(p); z++)
			{
				...
				for (int j = 0; j < lop_size_col; j++)
				{
					...
					{
						CC(in[0], eqiv[0]++, out[0]++, ROW(pp), R(p), ROW(pp));
					}
	...
	for(int y = 0; y < D(p); y++)
	{
		...
		for (int i = 0; i < D(pp); i++)
		{
			...
			for (int j = 0; j < lop_size_col; j++)
			{
				...
				for (int k = 0; k < lop_size_row; k++)
				{
					CC(in[1], eqiv[2]++, out[0]++, ROW(pp), R(p), ROW(pp));
				}
...
void loop::loop_tensor_with_same_chn_count(tensor* t, tensor* tt, tensor& out, lambda_inner_s3_with_stride f)
{
	elemt* p = t->get_p();
	elemt* pp = tt->get_p();
	elemt* op = out.get_p();
