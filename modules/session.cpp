...
#include "..\operation\tensor_ops\tensor_callback.h"
...
void session::make_layer_type_arr(session_type t, ly_type* tt)
{
...
void session::init()
{
	...
	for (int i = 0; i < LAYER_TYPE_NUM; i++)
	{
		...
		for (int k = 0; k < MAX_LAYER_COUNT; k++)
...
void mdls::session::add_layer(ly_type t)
{
...
void mdls::session::print_object(int start_index )
{

	for (int i = start_index; i < _occupied_size; i++)
...
void mdls::make_layer(layer** c, ly_type t)
{
	if (t == CONVOLUTION)
		*c = new convolution();
	else if (t == AFFINE)
...
(ly_type t, int cindex, int depth , int ninput, int rinput, int cinput,
...
layer *session::get_layer(layer_extended_enum ly, int index)
{
	try
	{
		if ( index_layer_by_type[ly][index] == -1)
		{
			throw index;
		}
	...
	{
		cout << "layer type " << inx << " doesn`t exists." << endl;
