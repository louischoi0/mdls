...
#define MAX_LAYER_COUNT 30
	...
	class session
		...
		public:
			...
			void init();
			void make_layer_type_arr(session_type t, ly_type* tt);
			...
			void set_layer_weight_tensor(ly_type t, int index , tensor* tns);
			void set_layer_input_tensor(ly_type t, int index, tensor* tns);
			...
			void print_object(int start_index);
			void print_object();
};

	void make_layer(layer** c, ly_type t);
